...
- **Form access setting matters.** Autumn Lake forms are restricted in Asana ("verify with your email" / organisation only), so they only render inside the embed for people already signed in or verified with Asana in that browser. Anonymous visitors are redirected to `app.asana.com/-/login`, which Asana refuses to frame (`frame-ancestors 'self'`), so the frame would show a browser error. The form page is therefore **open-first**: it shows an instruction card by default (no iframe), the primary button opens the form in a new tab at the non-embed URL (`embed=true` stripped) where Asana can run its email verification, and a secondary "Show embedded form" button mounts the official embed for staff who have already verified. The choice is remembered per form for the browser session. Changing who can access a form is an Asana-side setting, not something this app controls.
