...
- `/forms/[slug]` — form detail with `<AsanaEmbed>` + "Open in Asana" + sidebar. `AsanaEmbed` follows Asana's snippet exactly (`.asana-embed-container`, `.asana-embed-iframe`, `.asana-embed-footer` with the `.asana-embed-footer-logo` span whose image comes from Asana's stylesheet) and adds a short fallback line linking to the form in Asana. Forms are expected to be public-link Asana forms ("anyone with the link").
