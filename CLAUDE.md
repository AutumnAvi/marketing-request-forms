...
- `/forms/[slug]` — form detail with `<AsanaEmbed>` + "Open in Asana" + sidebar. `AsanaEmbed` is **open-first and progressive**: the default state is an instruction card (no iframe mounted) whose primary CTA opens the verify-friendly URL in a new tab; a secondary "Show embedded form" button reveals the official embed. The open URL is always derived from the catalog embed src by stripping `embed=true` (`asanaOpenUrl` in `src/lib/asana.ts`) — never a different Asana form. The revealed embed follows Asana's snippet exactly (`.asana-embed-container`, `.asana-embed-iframe`, `.asana-embed-footer` with the `.asana-embed-footer-logo` span whose image comes from Asana's stylesheet) plus a fallback line linking to the form in Asana and a "Hide" control. The reveal choice is remembered per form in `sessionStorage` (this browser, this session) — no cross-device auth is invented. Why: Autumn Lake forms are domain-gated; staff without an Asana seat verify their work email once per browser, and Asana refuses to run sign-in/verify inside a frame (CSP), so a blank iframe must never be the default.
